...
class MySQLWarObjectiveDatabase(
    ...
    fun insert(objective: com.minepalm.nations.war.WarObjective): CompletableFuture<Unit> {
        return database.executeAsync<Unit> { connection ->
            connection.prepareStatement(
                "INSERT INTO $table (`match_id`, `nation_id`, `user`, `objective_type`, `time`, `data`)" +
                        "VALUES(?, ?, ?, ?, ?, ?)"
            )
                .apply {
    ...
    fun select(matchId: Int): CompletableFuture<List<com.minepalm.nations.war.WarObjective>> {
        return database.executeAsync<List<com.minepalm.nations.war.WarObjective>> { connection ->
            connection.prepareStatement(
                "SELECT `match_id`, `nation_id`, `user`, `objective_type`, `data` FROM $table " +
                        "WHERE `match_id`=?"
            )
                .apply { setInt(1, matchId) }
                .executeQuery()
                .let {
                    mutableListOf<com.minepalm.nations.war.WarObjective>().apply {
                        while (it.next()) {
                            val nationId = it.getInt(2)
                            val uuid = UUID.fromString(it.getString(3))
                            val objectiveType =
                                it.getString(4).let { com.minepalm.nations.war.WarObjective.Type.valueOf(it) }
                            val time = it.getLong(5)
                            val data = it.getString(6)
                            add(
                                com.minepalm.nations.war.WarObjective(
                                    matchId,
                                    nationId,
                                    objectiveType,
                                    uuid,
