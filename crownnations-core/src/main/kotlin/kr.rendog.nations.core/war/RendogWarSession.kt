...
import kr.rendog.nations.core.mysql.MySQLWarSessionActiveDatabase
import kr.rendog.nations.core.mysql.MySQLWarSessionTimeDatabase
...
class RendogWarSession(
    ...
    override val unsafe: WarSession.Unsafe = UnsafeImpl(this,service,
        cacheImpl,
    ...
    private class CacheImpl : WarSession.Cache{
        ...
        internal fun setTime(time: WarTime){
            timeActual = time
        }

        internal fun markInvalidate(){
            invalidate = true
        }
    }
            ...
            return isActiveFuture.thenCompose { isActive ->
                return@thenCompose  if(isActive)
                    ...
                    timeDatabase.insert(session.gameId, time).thenCombineAsync(updateStatus, { _, _ ->
                        ...
                        val event = WarPostDeclarationEvent(server, session.data, time)
        ...
        override fun endGame(result: WarResult): CompletableFuture<Boolean> {
            ...
            return isActiveFuture.thenApplyAsync( { isActive ->
                if(isActive){
                    ...
                    service.root.localEventBus.invoke(WarEndEvent(service.root.network.host.name, session.gameId, result))
                }
                isActive
            }, executors)
        }
