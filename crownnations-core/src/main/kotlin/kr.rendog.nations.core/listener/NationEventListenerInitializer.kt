...
object NationEventListenerInitializer{
    ...
    fun init(service: NationService, executor: ExecutorService){
        service.remoteEventBus.run {
            ...
            addEventInitializer(NationAddMemberEvent::class.java, SyncAddMember(service, executor))
            addEventInitializer(NationRemoveMemberEvent::class.java, SyncRemoveMember(service, executor))
            addEventInitializer(NationSetRankEvent::class.java, SyncSetRank(service, executor))
            addEventInitializer(NationTransferEvent::class.java, SyncTransfer(service, executor))
            addEventInitializer(NationUpdateEvent::class.java, SyncUpdate(service, executor))
            addEventInitializer(NationGradeUpdateEvent::class.java, SyncGrade.Update(service))
            addEventInitializer(NationPromoteEvent::class.java, SyncGrade.Promote(service))

            addEventInitializer(TerritoryPostClaimEvent::class.java, SyncTerritory.Claim(service))
            addEventInitializer(TerritoryDecomposeEvent::class.java, SyncTerritory.Decompose(service))
